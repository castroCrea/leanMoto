...
export const GForceGauge: React.FC<Props> = ({ gForceX, gForceY, size = 200 }) => {
  ...
  const dotX = useSharedValue(cx);
  const dotY = useSharedValue(cy);

  useEffect(() => {
    const clampedX = Math.max(-MAX_G, Math.min(MAX_G, gForceX));
    const clampedY = Math.max(-MAX_G, Math.min(MAX_G, gForceY));
    const targetX = cx + (clampedX / MAX_G) * radius * 0.9;
    const targetY = cy - (clampedY / MAX_G) * radius * 0.9;

    dotX.value = withSpring(targetX, { damping: 15, stiffness: 200 });
    dotY.value = withSpring(targetY, { damping: 15, stiffness: 200 });
  }, [gForceX, gForceY]);

  const animDotProps = useAnimatedProps(() => ({
    cx: dotX.value,
    cy: dotY.value,
  }));
  ...
  const dotColor =
    totalG < 0.5 ? '#00D97E' : totalG < 1.0 ? '#00B4FF' : '#FF3A2F';
  ...
  return (
    <View style={[styles.container, { width: size, height: size }]}>
      <Svg width={size} height={size}>
        ...
        {/* Dot shadow / glow */}
        <Circle cx={cx} cy={cy} r={dotRadius + 4} fill={`${dotColor}22`}
          // use animatedProps via wrapper
        />
